...
<main class="mt-28">
    <div class="mx-auto max-w-[1330px] md:p-4">
        <div class="grid grid-cols-3 md:gap-4">
                        ...
                        <div v-for="article in list" :key="article._path" class="flex flex-col">
                            <div class="grid grid-cols-3 gap-4">
                                <div class="col-span-2 p-4">
                                    ...
                                    <p class="text-gray-700 mb-4">{{ desc(article) }}</p>
                                    <div class="mb-3">
                                    ...
                                    <div class="flex flex-wrap gap-2">
                                        <span v-for="tag in article.tags" :key="tag" class="bg-gray-200 rounded-full px-3 py-1 text-sm text-gray-700">{{ tag }}</span>
                                    </div>
                                </div>
                                <div class="col-span-1 p-4 flex justify-center items-center">
                                    <NuxtImg v-if="article.cover" :src="'/images/' + article.cover" :alt="article.title"  sizes="112px sm:112px md:112px" format="webp" loading="lazy" placeholder/>
                                    <NuxtImg
                                        v-else
            ...
            <div class="col-span-1 p-4 hidden lg:block border-l border-gray-200 sticky top-28 h-screen overflow-y-auto "  >
                <NuxtImg :src="config.public.author.image" :alt="config.public.author.name" class="rounded-full border border-gray-300 " width="100" height="100"/>

                <div class="font-semibold text-xl mt-2">{{config.public.author.name}}</div>
